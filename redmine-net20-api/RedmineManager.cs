    ...
    public class RedmineManager
    {
            ...
            uint version = 0)
        {
            var address = version == 0
                ? string.Format(WIKI_PAGE_FORMAT, _host, projectId, pageName)
                : string.Format(WIKI_VERSION_FORMAT, _host, projectId, pageName, version);
        ...
        public IList<WikiPage> GetAllWikiPages(string projectId)
        {
            ...
            return
                ExecuteDownloadList<WikiPage>(
                    string.Format(WIKI_INDEX_FORMAT, _host, projectId),
                    "GetAllWikiPages",
                    "wiki",
                    out totalCount);
        }
            ...
            using (var wc = CreateUploadWebClient())
            {
                try
                {
                    var response = wc.UploadData(
                        string.Format(FORMAT, _host, "uploads"),
                        data);
            ...
            out int totalCount) where T : class, new()
        {
            ...
            else if (type == typeof (IssueRelation))
            {
                ...
                address = string.Format(FORMAT, _host, urls[type]);
        ...
        {
            var type = typeof (T);
            return !urls.ContainsKey(type)
                ? null
                : ExecuteDownload<T>(
                    string.Format(REQUEST_FORMAT, _host, urls[type], id),
                    "GetObject<" + type.Name + ">",
                    parameters);
        }
            ...
            where T : class, new()
        {
            ...
            {
                address = string.Format(REQUEST_FORMAT, _host, urls[type], id);
            }
        ...
        protected WebClient CreateWebClient(NameValueCollection parameters)
        {
            ...
            {
                if (_cache != null) webClient.Credentials = _cache;
            }
